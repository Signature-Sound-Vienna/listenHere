...
let colormap;
let smoothedTempi = {};
const midiTempo = 120; // as generated by Verovio
const maxExpectedTempo = 320; // highest expected BPM
const windowRadius = 2; // size of smoothing window on either side of a given index
...
function smoothTempi(tempi, windowRadius = 2) {
	console.log("received tempi: ", tempi);
  if(refName) { 
    refGrid = alignmentGrids[refName];
    // for each tempo grid
    Object.keys(tempi).forEach(g => {
			console.log("g: ", g)
			// for each grid position, calculate the average tempo of a window of windowRadius grid cells on either side of the current position
			smoothedTempi[g] = tempi[g].map((p, ix) => {
				const left = ix <= windowRadius ? ix : windowRadius; // prevent out-of-bounds on left
				const right = tempi[g].length - 1 - ix > windowRadius ? tempi[g].length - 1 -ix : windowRadius; // prevent out-of-bounds on right
				const samples = tempi[g].slice(ix - left, ix + right + 1);
