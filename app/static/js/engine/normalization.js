...
// external touch is _seekAnalysis.clear() on a full reload, so _seekAnalysis is
// exported for that.

import { analyzeAudio } from "../audio-seek-index.js";
import { WindowedAudioPlayer } from "../windowed-audio-player.js";
import { wavesurfers, fileBlobs, _waveformPeaks } from "../listen.js";
...
export const _seekAnalysis = new Map(); // filename -> analyzeAudio() result (index, or null = native seek)
 ...
 */
export async function _maybeBuildWindowedPlayer(filename) {
 ...
 */
export function _setupNormGainNode(filename) {
...
export function _teardownNormGainNode(filename) {
  const player = _windowedPlayers[filename];
  if (player) {
...
export function _applyNormGain(normalize) {
