...
* **A second feedback round, same rules**: a GENERIC side slot (`?sideSlot=annotations`, `?sideSlotWidth=`) that turns each viewport into a two-column split — strips left, tenant right, "right" being each reader's own right since the slot rotates with its viewport — with the commentary panel as first tenant (plus a show/hide toggle for the group story) and the seam deliberately tenant-agnostic, because the optional Verovio score view would occupy exactly this split; and an audience union mode (`?audienceAll=1`) adding an "All" position to the switch that shows every audience's annotations at once, each chip marked with the audience it targets.
