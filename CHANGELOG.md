...
* Close-listening left/right marker navigation: widened the backward window (100ms → 800ms) so a quick double-press of Left reliably steps to the previous marker rather than re-selecting the just-reached one. Every annotation's region beginnings are now also navigable stops (alongside markers), not just those of the currently-active annotation.
