...
let colormap;
let tempi = {};
let midiTempo = 120; // as generated by Verovio
let maxExpectedTempo = 180; // highest expected BPM
try { 
...
/*
function timeToScoretime(t) { 
  return t / ((60 / midiTempo) * 1000)
}

function calculateTempi() {
  if(refName) { 
    refGrid = alignmentGrids[refName];
    // for each alignment grid
    Object.keys(alignmentGrids).forEach(g => {
        // determine, for each grid position:
        // 1. the difference in performance time
        // 2. the difference in score time (by reference to refGrid)
        // 3. based on these, the local tempo
        alignmentGrids.g.forEach((p, ix) => {
          if(ix === 0) { 
            tempi[g][ix] = 0; // 0 tempo at very start
          } else {
            deltaPerfTime = alignmentGrids[g][ix] - alignmentGrids[g][ix-1];
            deltaScoreTime = timeToScoretime(refGrid[g][ix]) - timeToScoretime(refGrid[g][ix-1]);
            tempi[g][ix] = (deltaScoreTime / deltaPerfTime) * 60000; // beats per minute
          }
        })
    })
  }
...
function onClickRenditionCheckbox(e)  {
  ...
  let tempoCurve = document.getElementById("waveform-"+ e.target.value+"-tempo");
  if(!checked) { 
    e.stopPropagation(); // hide from other handler
    waveform.style.display = "none";
    spectrogram.style.display = "none";
    tempoCurve.style.display = "none";
    ...
    if(document.getElementById("showTempoCurves").checked) 
      tempoCurve.style.display = "unset";
...
//      positionIndicatorStyle.display = document.getElementById("visalign").checked ? "unset" : "none";
      ...
      const tempoCtx = tempoCurveCanvas.getContext('2d');
      ...
      alignmentGrids[filename].forEach((gridPos, gridIx) => { 
        ...
        if(Object.keys(tempi).length) { 
          console.log("DRAWING")
          const tempoAbsoluteX = (gridIx / tempi[filename].length) * tempoCurveCanvas.width;
          tempoCtx.lineTo(tempoAbsoluteX, (tempi[filename][gridIx]/maxExpectedTempo) * tempoCurveCanvas.height);
        } else { 
